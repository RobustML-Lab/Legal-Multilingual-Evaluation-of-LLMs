...
languages = ["ar", "es", "ru", "zh"]
model_name = "mistral"  # Change to any model pulled in Ollama
file_path = "sampled_sdgs_one_label.jsonl"
...
for language in languages:
    csv_filename = f"mistral_{language}.csv"
    ...
    def build_prompt(text: str) -> str:
        if language == "en":
            return f"<s>[INST] Classify the following text into one SDG category from the list: [{subjects_str}]. Return only the most relevant category. Text: {text} [/INST]"
        elif language == "fr":
            return f"<s>[INST] Classez le texte suivant dans une seule catégorie des ODD parmi la liste : [{subjects_str}]. Retournez uniquement la catégorie la plus pertinente. Texte : {text} [/INST]"
        elif language == "ar":
            return f"<s>[INST] صنّف النص التالي ضمن فئة واحدة فقط من أهداف التنمية المستدامة من القائمة التالية: [{subjects_str}]قم بإرجاع رقم الفئة الأكثر صلة فقط ولا شيء آخر. نص: {text} [/INST]"
        elif language == "es":
            return f"<s>[INST] Clasifica el siguiente texto en una sola categoría de los ODS de esta lista: [{subjects_str}]. Devuelve solo la categoría más relevante. Texto: {text} [/INST]"
        elif language == "ru":
            return f"<s>[INST] Классифицируйте следующий текст в одну категорию из списка ЦУР: [{subjects_str}]. Верните только одну наиболее подходящую категорию. Текст: {text} [/INST]"
        elif language == "zh":
            return f"<s>[INST] 将以下文本分类为一个可持续发展目标类别，类别来自以下列表：[{subjects_str}]。仅返回最相关的一个类别。文本：{text} [/INST]"
    ...
    for _, row in df.iterrows():
        ...
        # if row.get("annotationId") == "ae049452-fc96-43ed-8c22-5e2715dca7af":
        #     toGo = True
        #
        # if not toGo:
        #     continue
